...
if (isset($postId)) {
  ...
  $postId = mysqli_real_escape_string($mysql, $postId);
  $query = "SELECT * FROM blogg.posts WHERE id='$postId' AND user_id='$userId';";
...
if (isset($_POST['submit'])) {
  ...
  if (!$article || strlen($article) < 10) {
    $form_valid = false;
    array_push($errors, 'The Article field is invalid! The field is mandatory and must be above 10 characters..');
  }
  
  if ($form_valid && isset($_FILES['input-upload']['error']) && $_FILES['input-upload']['error'] == 0 && $_FILES['input-upload']['name'] !== $post['image_upload'] && $_FILES['input-upload']['size'] !== filesize('images/posts/' . $post['image_upload']) && $_FILES['input-upload']['name'] !== end(explode('-*-', $post['image_upload']))) {
    if (check_image_post($_FILES)) {
      unlink('images/posts/' . $post['image_upload']);
  ...
  if ($form_valid) {
    ...
    $sql = "UPDATE posts SET title='$title', image_upload='$image_name', image_url='$imageURL', header='$header', text='$article' WHERE id='$postId' AND user_id='$userId';";
  ...
  <div class="col-lg-8 col-11 mt-5 mb-3 mx-auto px-0">
    ...
    <div class="border border-primary rounded py-3 px-md-5 px-3 d-block post">
      <form action="" method="POST" id="add-post" novalidate="novalidate" autocomplete="off" enctype="multipart/form-data">
        <h1 class="display-4 mx-auto mt-3 mb-5 py-2 text-primary text-center border-primary border-bottom w-75">Edit Your Post:</h1>
        <?php if ($errors) : ?>
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control form-control-lg" placeholder="ex: 'Tesla'" id="input-title" name="input-title" autofocus="autofocus" value="<?= $post['title']; ?>">
        </div>
        <div class="form-group">
          ...
          <div class="btn-group btn-group-toggle w-100" data-toggle="buttons">
            <label class="btn btn-secondary">
              <input type="radio" id="btn-url"<?= ($post['image_url']) ? 'checked' : '' ;?>> URL
            </label>
            <label class="btn btn-secondary active">
              <input type="radio" id="btn-upload" <?= ($post['image_upload']) ? 'checked' : '' ;?>> Upload
            </label>
          </div>
        </div>
        <div class="form-group mt-n2" id="field-url">
          <div class="form-group">
            <div class="input-group mb-3">
              <input type="text" class="form-control" aria-label="insert url" placeholder="ex: http://randonwebsite.com/image.png" name="input-url" value="<?= $post['image_url']; ?>">
        ...
        <div class="form-group mt-n2 d-none" id="field-upload">
          <div class="input-group">
            <div class="custom-file">
              ...
              <label class="custom-file-label" for="field-upload-element"><?= ($post['image_upload']) ? end(explode('-*-', $post['image_upload'])) : 'Upload image..'; ?></label>
            </div>
          </div>
        </div>
        <div class="form-group">
          ...
          <input type="text" class="form-control" placeholder="ex: 'My adventures in a electric car'" id="input-header" name="input-header" value="<?= $post['header']; ?>">
        </div>
        <div class="form-group">
          <label class="col-form-label ml-1" for="input-body">Article Text:</label>
          <div class="form-control"style="min-height: 45vh; font-family: sans-serif;" data-tiny-editor data-formatblock="no" data-fontname="no" data-justifyleft="no" data-justifycenter="no" data-justifyright="no" data-outdent="no" data-indent="no" data-remove-format="no"><?= htmlspecialchars_decode($post['text']); ?></div>
          </div>
          <textarea class="d-none" name="input-body" id="input-body" cols="0" rows="0"><?= htmlspecialchars_decode($post['text']); ?></textarea>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content p-3">
      ...
      <div class="modal-body">
        <p>By clicking on 'Cancel modification' you will be redirected back to the blog menu and your new post and all the changes made to it will not be saved !</p>
        <p>Otherwise clicking on 'Back' will get you back to your new post.
        </p>
      </div>
      <div class="modal-footer w-100">
        <button type="button" class="btn btn-outline-danger w-75 mx-0 cancel-modal">Cancel modification</button>
